...
task drive() {
	//Drive function is y = 0.78x
//	motorMultiplier = 0.78; // 100/128 - drive is linear from zero

	//Drive function is y = 0.86x - 10.36
	motorMultiplier = 0.86; // 100/(128-12) - drive
	...
	while(true) {
		...
		// Get joystick values
		x1 = joystick.joy1_x1 * motorMultiplier + yInt;
...
/*  SAVING THE OLD CODE JUST IN CASE*****************************
task armSlider() {
	motor[armLift] = 0; // the motor is set to not move
	while(true) {
		if(joy1Btn(5)) {
